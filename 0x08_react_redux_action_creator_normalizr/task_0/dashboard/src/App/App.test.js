...
describe('<MyComponent />', () => {
  ...
  it('verify that the Login component is not included', () => {
    const wrapper = shallow(<AppContext.Provider value={value}/>);
    wrapper.find("#login").simulate("click");
    expect(this.login).toBeCalled();
  });
  ...
  it('verify that the Login component is not included', () => {
    ...
    expect(this.logOut).toBeCalled();
  });

  it('verify that markNotificationAsRead works correctly', () => {
    const wrapper = shallow(<AppContext.Provider value={value}/>);
    wrapper.find("#logOut").simulate("click");
    expect(this.logOut).toBeCalled();
  });
